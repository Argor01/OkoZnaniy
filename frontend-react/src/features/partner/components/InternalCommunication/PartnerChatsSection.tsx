...
const getAvatarClass = (role: string) => {
  switch (role) {
    ...
    case 'partner': return styles.messageAvatarPartner;
    default: return styles.messageAvatarPartner;
  }
};
          ...
          {group.items.map((msg: any) => {
            ...
            return (
              <div key={msg.id} data-msg-id={msg.id} className={`${styles.messageRow} ${mine ? styles.messageMine : styles.messageTheirs}`}>
                <div className={`${styles.messageAvatar} ${getAvatarClass(msg.sender?.role)}`}>
                ...
                <div className={styles.messageContent}>
                  ...
                  <div className={`${styles.messageBubble} ${msg.is_pinned ? styles.messageBubblePinned : ''}`}>
                    {msg.file_url ? (
...
export const PartnerChatsSection: React.FC = () => {
  ...
  const handleFileUpload = async (file: File, msgText?: string) => {
    ...
    try {
      const sent = await uploadChatRoomFile(selectedRoom.id, file, msgText);
      if (sent && typeof sent === 'object' && sent.id) {
        setMessages(prev => [...prev, sent]);
      } else {
        await loadMessages(selectedRoom.id);
  ...
  const handlePinMessage = async (msgId: number) => {
    if (!selectedRoom) return;
    try {
      await pinChatRoomMessage(selectedRoom.id, msgId);
      await loadMessages(selectedRoom.id);
    } catch (error) {
