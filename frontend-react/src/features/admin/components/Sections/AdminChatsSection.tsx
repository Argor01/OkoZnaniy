...
import { Card, Button, Input, Avatar, Badge, Spin, Empty, Modal, Form, Select, Tabs, message as antMessage, Typography } from "antd";
        ...
        return (
          <div key={m.id} style={{ display: "flex", flexDirection: mine ? "row-reverse" : "row", gap: 8, alignItems: "flex-end" }}>
            ...
            <div style={{ maxWidth: "70%" }}>
              ...
              <div style={{ fontSize: 11, color: "#bfbfbf", marginTop: 2, textAlign: mine ? "right" : "left" }}>{fmt(String(m.created_at ?? ""))}</div>
            </div>
          </div>
        );
      })}
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ padding: "10px 16px", borderBottom: "1px solid #f0f0f0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div>
            <div style={{ fontWeight: 600 }}>{String(sel.name ?? "")}</div>
            ...
            return (
              <div key={c.id} onClick={() => { setSelChatId(c.id); setSelName(name); }}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#8c8c8c", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{String(c.last_message?.text ?? "Нет сообщений")}</div>
                </div>
                {c.last_message && <div style={{ fontSize: 11, color: "#bfbfbf", flexShrink: 0 }}>{fmt(String(c.last_message.created_at ?? ""))}</div>}
              </div>
            );
          })}
        </div>
          ...
          <Form.Item name="userId" label="Кому" rules={[{ required: true, message: "Выберите пользователя" }]}>
            <Select showSearch optionFilterProp="children" placeholder="Выберите пользователя">
              {(users as any[]).filter((u: any) => u.id !== uid).map((u: any) => (
                <Option key={u.id} value={u.id}>{String(u.first_name ?? "")} {String(u.last_name ?? "")} — {String(u.email ?? "")}</Option>
              ))}
            </Select>
          </Form.Item>
        </Form>
