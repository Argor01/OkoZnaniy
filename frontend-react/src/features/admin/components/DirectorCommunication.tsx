...
import { Card, Input, Button, Space, Typography, Divider, Empty, Tag, message, Grid, Tabs, Popover, Upload, Spin } from 'antd';
...
import { useDirectorCommunications, useDirectorCommunicationMessages, useDirectorCommunicationActions } from '@/features/admin/hooks/useDirectorCommunications';
...
const DirectorCommunication: React.FC = () => {
  ...
  const { communications, loading: communicationsLoading } = useDirectorCommunications();
  const { messages: currentMessages, loading: messagesLoading } = useDirectorCommunicationMessages(selectedCommunication);
  const { sendMessage } = useDirectorCommunicationActions();
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await sendMessage({ communicationId: selectedCommunication, message: messageText });
      setMessageText('');
    } catch (error) {
