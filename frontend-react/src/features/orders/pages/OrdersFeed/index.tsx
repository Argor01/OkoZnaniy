...
const OrdersFeed: React.FC = () => {
  ...
  const [available, own] = await Promise.all([
    ...
    // Лента — витрина свободных заказов. Как только клиент выбирает
    // исполнителя, заказ перестаёт быть свободным, и висеть в ленте он не
    // должен. Раньше он пропадал только после перезахода на страницу.
