#!/usr/bin/env python
"""
Скрипт для создания тестовых аккаунтов администраторов
Создает аккаунты директора, партнера и администратора
"""

import os
import django

os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'config.settings')
django.setup()

from apps.users.models import User
import uuid

# Данные пользователей для создания
users_data = [
    {
        'username': 'director',
        'email': 'director@test.com',
        'password': 'test123',
        'role': 'admin',  # Директор использует роль admin
        'first_name': 'Директор',
        'last_name': 'Тестовый',
        'is_staff': True,
        'is_superuser': True,
        'balance': 0,
        'frozen_balance': 0,
        'email_verified': True,
    },
    {
        'username': 'partner',
        'email': 'partner@test.com',
        'password': 'test123',
        'role': 'partner',
        'first_name': 'Партнер',
        'last_name': 'Тестовый',
        'balance': 5000,
        'frozen_balance': 0,
        'email_verified': True,
        'partner_commission_rate': 10.00,
        'total_referrals': 0,
        'active_referrals': 0,
        'total_earnings': 0,
    },
    {
        'username': 'admin',
        'email': 'admin@test.com',
        'password': 'test123',
        'role': 'admin',
        'first_name': 'Администратор',
        'last_name': 'Тестовый',
        'is_staff': True,
        'is_superuser': True,
        'balance': 0,
        'frozen_balance': 0,
        'email_verified': True,
    }
]

def main():
    print("Создание тестовых аккаунтов...")
    print("=" * 50)

    for user_data in users_data:
        username = user_data['username']
        email = user_data['email']
        password = user_data.pop('password')
        
        # Проверяем, существует ли пользователь
        user, created = User.objects.get_or_create(
            username=username,
            defaults=user_data
        )
        
        if created:
            user.set_password(password)
            user.save()
            print(f"✅ Создан пользователь: {username}")
            print(f"   Email: {email}")
            print(f"   Роль: {user.role}")
            print(f"   Пароль: {password}")
            
            # Генерируем реферальный код для партнера
            if user.role == 'partner' and not user.referral_code:
                user.referral_code = str(uuid.uuid4())[:8].upper()
                user.save()
                print(f"   Реферальный код: {user.referral_code}")
            
            print()
        else:
            # Обновляем существующего пользователя
            for key, value in user_data.items():
                setattr(user, key, value)
            user.set_password(password)
            user.save()
            
            # Генерируем реферальный код для партнера если его нет
            if user.role == 'partner' and not user.referral_code:
                user.referral_code = str(uuid.uuid4())[:8].upper()
                user.save()
            
            print(f"🔄 Обновлен пользователь: {username}")
            print(f"   Email: {email}")
            print(f"   Роль: {user.role}")
            print(f"   Пароль: {password}")
            
            if user.role == 'partner' and user.referral_code:
                print(f"   Реферальный код: {user.referral_code}")
            
            print()

    print("=" * 50)
    print("Все аккаунты созданы/обновлены!")
    print()
    print("Данные для входа:")
    print("📧 Директор:")
    print("   Email: director@test.com")
    print("   Пароль: test123")
    print("   Роль: admin (директор)")
    print()
    print("🤝 Партнер:")
    print("   Email: partner@test.com")
    print("   Пароль: test123")
    print("   Роль: partner")
    print()
    print("⚙️ Администратор:")
    print("   Email: admin@test.com")
    print("   Пароль: test123")
    print("   Роль: admin")
    print()
    print("Все аккаунты готовы к использованию!")

if __name__ == '__main__':
    main()